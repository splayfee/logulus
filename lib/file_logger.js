 ...
 * @author <a href="mailto:dlatour@apple.com">dlatour</a>
 */
...
function FileLogger(options) {
  ...
  this.directory = Assert.setDefault(options.directory, ".");
  this.baseName = Assert.setDefault(options.baseName, "Logan-Bootstrap");
  this.timestamp = options.timestamp;
  this.saveCount = Assert.setDefault(options.saveCount, 3);
    ...
    if (this._firstTime) {
      ...
      if (self.timestamp) {
        if (Assert.isFunction(self.timestamp)) {
          ...
          var date = new Date();
          return util.format("%s.%s", date.toLocaleTimeString(), ("000" + date.getMilliseconds()).slice(-3));
        }
      }
