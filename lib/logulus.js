...
Logulus._loadEnvironmentConfig = function () {
    ...
    if (configPath && fs.existsSync( configPath )) {
        try {
            ...
            console.log( "Logulus: Unable to environment configuration file." );
        }
    }
    return result;
};
...
Logulus._loadConfig = function () {
    ...
    if (!configFilePath) {
        configFilePath = Logulus._loadDefaultConfig();
    }
    if (!configFilePath) {
        Logulus.config = {};
    }
    ...
    Logulus.config.colors = Assert.setDefault( Logulus.config.colors, {
        debug: "green",
        info: "green",
        warn: "yellow",
        error: "red"
    ...
    Logulus.timestampSettings = Assert.setDefault( Logulus.timestampSettings, {
        includeDate: true,
...
Logulus.create = function ( moduleId ) {
    ...
    return new LogulusProxy( moduleId, Logulus );
};
