...
Logulus._init = function () {
    ...
    var transportDefinitions = Assert.setDefault(Logulus.config.transports,
        [
            ...
            {"type":"file", "name": "logulus-file", "directory": "logs", "saveCount": 5, "level": "debug", "handleExceptions": true, "silent": false}
        ]
    );
    ...
    if (Assert.isArray( transportDefinitions )) {
        ...
        transportDefinitions.forEach( function ( item ) {
            var levelKeys, minLevel, minLevelName, transport, transportConfig;
            levelKeys = Object.keys( Logulus.config.levels );
            minLevel = Number.MAX_VALUE;
            minLevelName = null;
            transport = null;
            transportConfig = {timestamp: Logulus.localTimestamp};
            levelKeys.forEach( function ( lvlKey ) {
                if (Logulus.config.levels[lvlKey] < minLevel) {
                    minLevel = Logulus.config.levels[lvlKey];
                    minLevelName = lvlKey;
                }
            } );
    ...
    var loggerConfig = {
        transports: Logulus.config.transports,
        exitOnError: Logulus.config.exitOnError
    };
    Logulus.logger = new Logger( loggerConfig );
...
Logulus.create = function ( moduleId ) {
    if (!moduleId) {
        moduleId = (typeof module !== "undefined" && module.filename) ? module.filename : "unknown";
    }
...
Logulus.isAllowed = function ( moduleId, level ) {
    ...
    var includeAll = false, result = false;
