...
Logan._loadDefaultConfig = function() {
  ...
  try {
    configPath = path.resolve(__dirname, util.format("../logan_defaults.json"));
  } catch(error) {
    configPath = undefined;
  }
...
Logan._loadConfig = function() {
  ...
  Logan.config.transports = [{"name":"console", "colorize": true, "level": "debug", "handleExceptions": true, "silent": false}, {"name":"file", "saveCount": 5, "level": "debug", "handleExceptions": true, "silent": false}];
  Logan.config.baseName = Assert.setDefault(Logan.config.baseName, "default");
  ...
  Logan.config.includeFilters = Assert.setDefault(Logan.config.includeFilters, [{"pattern": "*", "levels": ["debug", "info", "warn", "error"]}]);
  Logan.config.levels = Assert.setDefault(Logan.config.levels, {debug: 0, info: 1, warn: 2, error: 3});
